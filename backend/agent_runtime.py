...
def _collect_mastery_update(result: dict[str, Any]) -> dict[str, Any] | None:
    """Extract the mastery_update from grade_answer tool responses.

    The grader-critic subagent calls grade_answer which returns a structured
    mastery_update with proficiency_before, proficiency_after, and status info.
    This is the ONLY authoritative source for mastery numbers — the LLM prose
    should never be trusted for these values.

    Priority: check the request-scoped sink first (captures subagent tool calls
    that never appear in the orchestrator's message list), then fall back to
    scanning the orchestrator's messages.
    """
    # Primary source: the request-scoped sink captures grade_answer from subagents.
    sink_grade = collected_mastery_update()
    if sink_grade:
        return sink_grade

    # Fallback: scan orchestrator's messages (in case grade_answer was called directly).
    for message in result.get("messages", []) or []:
        if getattr(message, "type", None) != "tool":
            continue
        content = getattr(message, "content", None)
        ...
        # Check if this is a grade_answer response (has mastery_update)
        mastery_update = payload.get("mastery_update")
        if isinstance(mastery_update, dict):
            # Include related fields for full context
            return {
                "mastery_update": mastery_update,
                "skill": payload.get("skill"),
                "score": payload.get("score"),
                "verdict": payload.get("verdict"),
                "covered_points": payload.get("covered_points"),
...
def _format(result: dict[str, Any], *, learner_id: str, tenant_id: str, thread_id: str) -> dict[str, Any]:
    ...
    response = {**base, "message": _final_message(result), "source_refs": _collect_source_refs(result)}
    # Include mastery_update if this turn graded an answer — this is the authoritative
    # source for mastery numbers, not the LLM prose which may fabricate percentages.
    grade_data = _collect_mastery_update(result)
    if grade_data:
        response["grade"] = grade_data
    return response
