...
def _maybe_coach_note(skill: str, verdict: str, covered: list[str], missed: list[str]) -> dict[str, Any]:
    """Optionally add an LLM coaching tip. Deterministic fields are never changed.
    ...
    Supports both Qwen and Nebius Token Factory providers.
    """
    from .settings import get_settings

    settings = get_settings()
    if not settings.llm_coaching_enabled:
        return {}
    ...
    # Try Nebius first if configured, then Qwen
    if settings.llm_provider == "nebius":
        from . import nebius_provider
        if nebius_provider.available():
            try:
                note = nebius_provider.chat(messages, max_tokens=120)
                return {"coach_note": note.strip(), "coach_model": settings.nebius_llm_model}
            except nebius_provider.LLMUnavailable:
                pass
    ...
    if not llm_provider.available():
        return {}
    try:
        note = llm_provider.chat(messages, max_tokens=120)
    except llm_provider.LLMUnavailable:
        return {}
    return {"coach_note": note.strip(), "coach_model": settings.qwen_llm_model}
