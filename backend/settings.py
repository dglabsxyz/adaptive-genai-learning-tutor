...
def _llm_provider_default() -> str:
    """Use Nebius/Qwen when a key is present, else stay deterministic.

    An explicit ``TUTOR_LLM_PROVIDER`` always wins so operators can force a mode.
    Priority: explicit env > Nebius > Qwen > deterministic.
    """
...
class AppSettings(BaseModel):
    ...
    llm_provider: Literal["deterministic", "openai", "qwen", "nebius"] = "deterministic"
    ...
    # Nebius Token Factory (OpenAI-compatible) configuration.
    nebius_api_key: str | None = None
    nebius_base_url: str = "https://api.tokenfactory.nebius.com/v1"
    nebius_llm_model: str = "meta-llama/Meta-Llama-3.1-70B-Instruct-fast"
    nebius_embedding_model: str = "BAAI/bge-en-icl"
    nebius_timeout_seconds: float = 60.0
...
def get_settings() -> AppSettings:
    ...
    return AppSettings(
        ...
        nebius_llm_model=os.getenv("NEBIUS_LLM_MODEL", "meta-llama/Meta-Llama-3.1-70B-Instruct-fast"),
        nebius_embedding_model=os.getenv("NEBIUS_EMBEDDING_MODEL", "BAAI/bge-en-icl"),
        nebius_timeout_seconds=_float_env("NEBIUS_TIMEOUT_SECONDS", 60.0),
